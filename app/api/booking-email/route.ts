...
function buildHtml(destination: string, flight: Flight, hotel: Hotel, flightDisplay: string | null, hotelDisplay: string | null, total: string, receiptUrl: string | null): string {
  ...
  return `<div style="font-family:system-ui,sans-serif;max-width:560px;margin:0 auto;color:#15110c">
    ...
    ${total ? `<div style="display:flex;justify-content:space-between;background:#15110c;color:#fff;border-radius:14px;padding:14px 18px;font-weight:600"><span>Total paid</span><span>${esc(total)}</span></div>` : ""}
    ${receiptUrl ? `<p style="margin:16px 0 0"><a href="${esc(receiptUrl)}" style="display:inline-block;background:#fff;border:1px solid #ddd;border-radius:10px;padding:10px 16px;color:#15110c;text-decoration:none;font-weight:600;font-size:14px">View your Stripe receipt &#8599;</a></p>` : ""}
    <p style="color:#999;font-size:12px;margin-top:22px">Your payment receipt is issued by Stripe. The booking numbers above are real confirmations from our flight and hotel partners. Live ticketing switches on with our travel licence. Reply any time and a real person will read it.</p>
  </div>`;
}
...
export async function POST(req: Request) {
  ...
  let body: {
    email?: string; destination?: string; receiptUrl?: string | null;
  ...
  const html = buildHtml(destination, body.flight ?? null, body.hotel ?? null, body.flightDisplay ?? null, body.hotelDisplay ?? null, body.total || "", body.receiptUrl ?? null);
